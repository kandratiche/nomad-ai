...
function buildLeafletHTML(stops: TimelineStop[]): string {
    ...
    const markersJS = validStops
        ...
        }).addTo(map).bindPopup('<b>${i + 1}. ${stop.title.replace(/'/g, "\\'")}</b>${stop.time ? "<br/>" + stop.time : ""}');
    ...
    // stops data for badge positioning
  ...
  <script>
    ...
    function updateRoute(route, mode) {
      ...
      if (route.geometry && route.geometry.length > 0) {
        addLayer(L.polyline(route.geometry, {color: '#7C3AED', weight: 5, opacity: 0.85, lineCap: 'round', lineJoin: 'round'}));
      } else {
        var fallback = stops.map(function(s) { return [s.lat, s.lng]; });
        addLayer(L.polyline(fallback, {color: '#A78BFA', weight: 4, opacity: 0.8, dashArray: '8,12', lineCap: 'round'}));
      }

      // badges between stops
      var icon = mode === 'car' ? '🚗' : '🚶';
      (route.segments || []).forEach(function(seg, i) {
        ...
        var dur = seg.durFormatted;
        addLayer(L.marker([midLat, midLng], {
          icon: L.divIcon({
    ...
    document.addEventListener('message', function(e) {
      try {
        ...
        updateRoute(data.route, data.mode);
      } catch(err) {}
    });
    // For iOS WebView
...
export default function RouteMapModal({ visible, stops, onClose }: Props) {
    ...
    // Build HTML once from stops only — no route dependency
    ...
    // Send route to already-loaded map via postMessage (no map reload)
    useEffect(() => {
        if (!visible) return;

        // Prepare serializable route with pre-formatted durations
        const serializableRoute = route
            ? {
                ...route,
                segments: route.segments.map((seg) => ({
                    ...seg,
                    durFormatted: formatDuration(
                        mode === "foot" ? seg.durationMinutes * 10 : seg.durationMinutes
                    ),
                })),
            }
            : null;

        const msg = JSON.stringify({ route: serializableRoute, mode });
        ...
        if (Platform.OS === "web") {
            ...
            // Post to WebView — small delay to ensure it's loaded
            setTimeout(() => {
                        ...
                        <WebViewComponent
                            ...
                            renderLoading={() => (
                                <View style={styles.fallback}>
                                    ...
                                    <Text style={styles.fallbackText}>Загрузка карты...</Text>
                                </View>
                            )}
                        />
                    ) : (
                                    ...
                                    <View style={styles.segmentRow}>
                                        ...
                                        <View style={styles.segmentBadge}>
                                            ...
                                            <Text style={styles.segmentText}>
                                                {formatDuration(mode === "foot" ? segment.durationMinutes * 10 : segment.durationMinutes)} · {formatKm(segment.distanceKm)}
                                            </Text>
                                        </View>
                                    </View>
                                )}
